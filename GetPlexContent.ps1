...
#get all libraries
$tvArchivedShows = Get-PlexContentInLibrary -URI $PlexURL -PlexToken $PlexAuthToken -Search '*Archives*' -Type Show -AllDetails
$tvArchivedShows | select title, studio, viewcount | Sort-Object @{e={$_.viewcount -as [int]}} -Descending

$tvkids = Get-PlexContentInLibrary -URI $PlexURL -PlexToken $PlexAuthToken -Search '*Kids*' -Type Show -AllDetails
$tvkids | select title, studio, viewcount | Sort-Object @{e={$_.viewcount -as [int]}} -Descending

$tvshows = Get-PlexContentInLibrary -URI $PlexURL -PlexToken $PlexAuthToken -Search '*All*' -Type Show -AllDetails
$tvshows | select title, studio, viewcount | Sort-Object @{e={$_.viewcount -as [int]}} -Descending

$tvpremium = Get-PlexContentInLibrary -URI $PlexURL -PlexToken $PlexAuthToken -Search '*Premium*' -Type Show -AllDetails
...
$RecentlyAdded | select title, studio, viewcount | Sort-Object @{e={$_.viewcount -as [int]}} -Descending

#$PlexAdmin = Invoke-WebRequest "$PlexExternalURL/users/account" -Headers @{'accept'='application/json';'X-Plex-Token'=$PlexAuthToken}

$AllMovies = Get-PlexContentInLibrary -URI $PlexURL -PlexToken $PlexAuthToken -Search '*Collection*' -Type Movie -AllDetails
$AllMovies | select title, studio, viewcount | Sort-Object @{e={$_.viewcount -as [int]}} -Descending
